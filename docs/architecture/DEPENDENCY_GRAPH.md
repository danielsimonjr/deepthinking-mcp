...
<!-- Generated by npx tsx tools/create-dependency-graph/create-dependency-graph.ts (npm run docs:deps) — its freshness is governed by its own generator, and its
     methodology differs from repo_map's (see DRIFT_REPORT.md, "reconciliation").
     repo_map's gate cannot meaningfully verify another tool's numbers. -->
