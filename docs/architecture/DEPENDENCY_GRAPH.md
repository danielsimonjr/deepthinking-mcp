...
### `src/export/visual/session-graph.ts` - Session-level graph rendering (v9.4.1)
...
| `../visual-exporter.js` | `VisualExporter` | Import |
| `../../../utils/sanitization.js` | `escapeLatex` | Import |
...
| `./services/RecommendationService.js` | `buildModeRecommendation` | Import |
...
| `../../tools/thinking.js` | `ThinkingToolInput` | Import (type-only) |
| `./ModeHandler.js` | `ModeHandler, ValidationResult, HandlerValidationError, ValidationWarning, ModeEnhancements, validationSuccess, validationFailure, createValidationWarning` | Import |
...
| `../../tools/thinking.js` | `ThinkingToolInput` | Import (type-only) |
| `./ModeHandler.js` | `ModeHandler, ValidationResult, HandlerValidationError, ValidationWarning, ModeEnhancements, validationSuccess, validationFailure, createValidationError, createValidationWarning` | Import |
...
**Exports:**
- Interfaces: `ValidationResult`, `HandlerValidationError`, `ValidationWarning`, `DetectedArchetype`, `ModeEnhancements`, `ModeHandler`, `ModeStatus`
...
| `../../tools/thinking.js` | `ThinkingToolInput` | Import (type-only) |
| `./ModeHandler.js` | `ModeHandler, ValidationResult, HandlerValidationError, ValidationWarning, ModeEnhancements, DetectedArchetype, validationSuccess, validationFailure, createValidationError, createValidationWarning` | Import |
...
| `./handlers/ModeHandler.js` | `ModeHandler, ValidationResult, HandlerValidationError, ValidationWarning, ModeEnhancements, ModeStatus, DetectedArchetype, validationSuccess, validationFailure, createValidationError, createValidationWarning` | Re-export |
...
### `src/proof/advisory.ts` - Advisory proof-analysis wrapper

**Internal Dependencies:**
| File | Imports | Type |
|------|---------|------|
| `../types/core.js` | `Thought` | Import (type-only) |
| `../types/session.js` | `AdvisoryProofAnalysis, ProofAnalysisSource` | Import (type-only) |
| `../types/modes/mathematics.js` | `GapAnalysis, Inconsistency, ProofDecomposition, ProofStrategy, Theorem` | Import (type-only) |
| `../types/modes/formallogic.js` | `LogicalProof` | Import (type-only) |
| `../types/core.js` | `ThinkingMode` | Import |
| `./decomposer.js` | `ProofDecomposer, ProofStep` | Import |
...
- Functions: `analyzeProofAdvisory`
- Constants: `MAX_PROOF_STEPS`, `MAX_PROOF_ATOMS`, `MAX_PROOF_GAPS`, `MAX_PROOF_ASSUMPTIONS`, `MAX_PROOF_INCONSISTENCIES`, `MAX_PROOF_CYCLES`, `MAX_PROOF_NOTES`
...
| `../export/visual/session-graph.js` | `renderSessionGraph, SessionGraphNode, SessionGraphEdge` | Import |
| `../utils/sanitization.js` | `escapeLatex` | Import |
...
| File | Imports | Type |
|------|---------|------|
| `../types/modes/recommendations.js` | `ModeRecommender, ProblemCharacteristics` | Import |
| `../taxonomy/advisory.js` | `renderReasoningTypeAdvice, suggestReasoningTypesAdvisory` | Import |

**Exports:**
- Interfaces: `RecommendModeInput`
- Functions: `buildModeRecommendation`
...
| `../utils/errors.js` | `InvalidModeError, ResourceLimitError, SessionNotFoundError` | Import |
...
### `src/taxonomy/advisory.ts` - Advisory reasoning-type wrapper

**Internal Dependencies:**
| File | Imports | Type |
|------|---------|------|
| `./reasoning-types.js` | `REASONING_TAXONOMY` | Import |
| `./classifier.js` | `TaxonomyClassifier, ThoughtClassification` | Import |
| `./suggestion-engine.js` | `SuggestionEngine, ProblemCharacteristics, ReasoningSuggestion` | Import |
| `../types/modes/recommendations.js` | `ProblemCharacteristics` | Import (type-only) |

**Exports:**
- Interfaces: `SuggestedReasoningType`, `ProblemClassification`, `TaxonomyAdviceAvailable`, `TaxonomyAdviceUnavailable`, `TaxonomyAdviceInput`, `TaxonomyAdviceDeps`
- Functions: `toTaxonomyCharacteristics`, `suggestReasoningTypesAdvisory`, `renderReasoningTypeAdvice`
- Constants: `MAX_TAXONOMY_SUGGESTIONS`, `MAX_TAXONOMY_RATIONALE`, `MAX_TAXONOMY_WARNINGS`, `MAX_TAXONOMY_KEYWORDS`
...
| File | Imports | Type |
|------|---------|------|
| `../export/profiles.js` | `EXPORT_PROFILES, listExportProfileIds` | Import |
...
| File | Imports | Type |
|------|---------|------|
| `./shared.js` | `ConfidenceSchema, PositiveIntSchema, SessionActionEnum, ExportFormatEnum, ExportProfileEnum, LevelEnum, IdSchema, SessionIdSchema, TextSchema, ThoughtTextSchema, IdArraySchema` | Import |
...
|------|---------|------|
| `../base.js` | `BaseThoughtSchema` | Import |
| `../shared.js` | `ConfidenceSchema, TimeUnitEnum, TemporalConstraintEnum, TemporalRelationEnum, EventTypeEnum, IdSchema, NameSchema, TextSchema, IdArraySchema, boundedRecord` | Import |
| `../../../utils/sanitization.js` | `MAX_LENGTHS` | Import |
...
**Exports:**
- Functions: `boundedArray`, `boundedRecord`
- Constants: `ConfidenceSchema`, `PositiveIntSchema`, `IdSchema`, `SessionIdSchema`, `NameSchema`, `TextSchema`, `ThoughtTextSchema`, `IdArraySchema`, `TextArraySchema`, `LevelEnum`, `ImpactEnum`, `ExportFormatEnum`, `SessionActionEnum`, `ExportProfileEnum`, `ProofTypeEnum`, `TimeUnitEnum`, `TemporalConstraintEnum`, `TemporalRelationEnum`, `EventTypeEnum`, `TransformationEnum`, `ShannonStageEnum`, `EntitySchema`, `DescribedEntitySchema`
...
|------|---------|------|
| `../utils/sanitization.js` | `MAX_LENGTHS` | Import |
| `./schemas/shared.js` | `boundedRecord` | Import |
...
| File | Imports | Type |
|------|---------|------|
| `./session.js` | `AdvisoryProofAnalysis, AdvisoryValidation` | Import (type-only) |
...
| `../modes/handlers/ModeHandler.js` | `type ModeHandler, type ValidationResult, type HandlerValidationError, type ValidationWarning, type ModeEnhancements, type ModeStatus, validationSuccess, validationFailure, createValidationError, createValidationWarning` | Re-export |
...
### `src/validation/advisory.ts` - Advisory validation wrapper

**Internal Dependencies:**
| File | Imports | Type |
|------|---------|------|
| `../types/core.js` | `Thought` | Import (type-only) |
| `../types/session.js` | `AdvisoryValidation, ValidationIssue, ValidationResult` | Import (type-only) |
| `./constants.js` | `ValidationContext` | Import (type-only) |
| `./validator.js` | `ThoughtValidator` | Import |

**Exports:**
- Interfaces: `AdvisoryValidatorLike`
- Functions: `validateAdvisory`
- Constants: `MAX_ADVISORY_ISSUES`, `MAX_ADVISORY_SUGGESTIONS`
...
| `../config/index.js` | `getConfig` | Import |
| `./validators/index.js` | `getSupportedModes, getValidatorForMode` | Import |
| `./constants.js` | `ValidationContext` | Import (type-only) |
...
- **Type-only cycles**: 58 (safe, no runtime impact)

### Type-Only Circular Dependencies

These cycles only involve type imports and are safe (erased at runtime):

- src/types/session.ts -> src/types/core.ts -> src/types/session.ts
...
graph TD
    ...
    subgraph Services
        N24[ExportService]
        N25[RecommendationService]
        N26[ThoughtFactory]
    end

    subgraph Session
        N27[manager]
        N28[SessionMetricsCalculator]
        N29[file-store]
        N30[interface]
    end
