...
- **Made the lint and format-check gates in `test.yml` real.** Both the "Run linter" (`npm run lint`) and "Run format check" (`npm run format:check`) steps carried `continue-on-error: true`, so the job reported success even when they failed — decorative gates nobody ever saw fail. `npm run format:check` was in fact failing (237 files not Prettier-formatted); `npm run lint` was passing (0 errors, 397 pre-existing `warn`-level findings under the repo's intentionally lenient `eslint.config.js`). Ran `npm run format` to reformat all 237 files (formatting-only changes, no logic touched) and removed `continue-on-error: true` from both steps now that they exit 0 for real.
