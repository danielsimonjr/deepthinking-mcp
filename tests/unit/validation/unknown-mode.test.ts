 ...
 * the legacy tool. It is reachable for a library caller and for the five
 * `ThinkingMode` members with no validator entry (recursive, modal,
 * stochastic, constraint, custom).
 */
...
describe('validation for a mode with no validator', () => {
  ...
  it('reports a warning, not an error, and stays valid', async () => {
    const result = await validator.validate(makeThought(ThinkingMode.RECURSIVE));
  ...
  it('suggests the modes that actually have validators', async () => {
    const result = await validator.validate(makeThought(ThinkingMode.MODAL));
    ...
    expect(suggestion).not.toContain('modal');
  });
  ...
  it('covers every ThinkingMode member without a validator', () => {
    ...
    expect(missing.sort()).toEqual(
      ['constraint', 'custom', 'modal', 'recursive', 'stochastic'].sort(),
    );
  });
});
