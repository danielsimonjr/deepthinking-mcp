...
async function handleAddThought(input, _toolName) {
    ...
    let mode = input.mode || ThinkingMode.HYBRID;
    ...
    if (mode === ThinkingMode.MATHEMATICS && isDecompositionThoughtType(input.thoughtType)) {
        try {
            ...
            const analysisResult = mathEngine.analyzeForThoughtType(proofInput, input.thoughtType, input.theorem);
...
async function handleSwitchMode(input) {
    ...
    const session = await modeRouter.switchMode(input.sessionId, input.newMode, 'User requested mode switch');
    return {
        content: [
            {
                type: 'text',
...
async function handleRecommendMode(input) {
    ...
    if (input.problemCharacteristics) {
        const response = modeRouter.getRecommendations(input.problemCharacteristics, input.includeCombinations || false);
        return {
            content: [{
                    type: 'text',
                    text: response
