...
function validateEvidenceChain(
  ...
  const calculatedTotal = chain.observations.reduce(
    (sum, obs) => sum + obs.decibans,
    0,
  );
  ...
  const expectedConclusion =
    chain.totalDecibans >= chain.confirmationThreshold
      ? "confirmed"
...
function validateDecibanEvidence(
  thought: CryptanalyticThought,
...
function validateKeySpaceAnalysis(
  thought: CryptanalyticThought,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (Math.abs(total - eliminated - remaining) > 1) {
    issues.push({
      ...
      thoughtNumber: thought.thoughtNumber,
      description:
        "Key space arithmetic inconsistent: remaining ≠ total - eliminated",
  ...
  if (
    remaining > 0 &&
    Math.abs(expectedFactor - analysis.reductionFactor) / expectedFactor > 0.01
  ) {
    issues.push({
      severity: "warning",
      thoughtNumber: thought.thoughtNumber,
...
function validateFrequencyAnalysis(
  thought: CryptanalyticThought,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  for (const crib of cribAnalyses) {
    ...
    if (crib.crib.length !== crib.ciphertext.length) {
      issues.push({
        ...
        suggestion:
          "Crib and corresponding ciphertext should have the same length",
        category: "structural",
      });
    }
    ...
    if (!crib.isViable && crib.contradictions.length === 0) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Crib marked as non-viable but no contradictions documented",
