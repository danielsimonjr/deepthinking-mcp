  ...
  validate(
    ...
    if (thought.gtCategories && thought.gtCategories.length > 0) {
      ...
      for (const category of thought.gtCategories) {
        ...
        if (
          category.saturation &&
          !validSaturationLevels.includes(category.saturation)
        ) {
          issues.push({
            severity: "warning",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.codingProgress) {
      if (
        thought.codingProgress.segmentsCoded >
        thought.codingProgress.totalSegments
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
      ...
      if (thought.codingProgress.percentComplete !== undefined) {
        if (
          thought.codingProgress.percentComplete < 0 ||
          thought.codingProgress.percentComplete > 100
        ) {
          issues.push({
            severity: "error",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.rigorAssessment) {
      ...
      if (rigor.confirmability?.rating !== undefined) {
        issues.push(
          ...validateProbability(
            thought,
            rigor.confirmability.rating,
            "confirmability rating",
          ),
        );
      }
    }
