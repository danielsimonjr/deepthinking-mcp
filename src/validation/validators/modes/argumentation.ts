  ...
  validate(
    ...
    if (thought.grounds && thought.grounds.length > 0) {
      ...
      for (const ground of thought.grounds) {
        ...
        if (
          ground.sufficiency &&
          !validSufficiencyLevels.includes(ground.sufficiency)
        ) {
          issues.push({
            severity: "info",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.rebuttals && thought.rebuttals.length > 0) {
      ...
      for (const rebuttal of thought.rebuttals) {
        ...
        if (
          rebuttal.strength &&
          !validRebuttalStrengths.includes(rebuttal.strength)
        ) {
          issues.push({
            severity: "info",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (
          rebuttal.targetElement &&
          !validTargets.includes(rebuttal.targetElement)
        ) {
          issues.push({
            severity: "warning",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (rebuttal.response?.effectiveness !== undefined) {
          issues.push(
            ...validateProbability(
              thought,
              rebuttal.response.effectiveness,
              "rebuttal response effectiveness",
            ),
          );
        }
    ...
    if (thought.dialectic) {
      ...
      if (
        thought.dialectic.resolution &&
        !validResolutions.includes(thought.dialectic.resolution)
      ) {
        issues.push({
          severity: "warning",
          thoughtNumber: thought.thoughtNumber,
    ...
    if (
      thought.rhetoricalStrategies &&
      thought.rhetoricalStrategies.length > 0
    ) {
      const validAppeals = ["ethos", "pathos", "logos"];
      const validAppropriateness = [
        "appropriate",
        "questionable",
        "inappropriate",
      ];
      ...
      for (const strategy of thought.rhetoricalStrategies) {
        ...
        if (
          ...
          !validAppropriateness.includes(strategy.appropriateness)
        ) {
          issues.push({
            severity: "info",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (strategy.effectiveness !== undefined) {
          issues.push(
            ...validateProbability(
              ...
              "rhetorical strategy effectiveness",
            ),
          );
        }
      }
        ...
        if (arg.overallStrength !== undefined) {
          issues.push(
            ...validateProbability(
              thought,
              arg.overallStrength,
              `argument "${arg.id}" overall strength`,
            ),
          );
        }
      }
    ...
    if (thought.currentArgument) {
      if (thought.currentArgument.overallStrength !== undefined) {
        issues.push(
          ...validateProbability(
            thought,
            thought.currentArgument.overallStrength,
