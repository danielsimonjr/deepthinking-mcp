  ...
  validate(thought: Thought, _context: ValidationContext): ValidationIssue[] {
    ...
    if (!hasConstraintContent) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Constraint-based reasoning typically discusses constraints explicitly",
        suggestion:
          "Consider making explicit references to constraints being satisfied or violated",
        category: "structural",
      });
    }
    ...
    if ("solution" in thought && thought.thoughtNumber > 1) {
      const hasSatisfaction =
        thought.content.toLowerCase().includes("satisf") ||
