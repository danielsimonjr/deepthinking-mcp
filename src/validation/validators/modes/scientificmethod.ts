...
export class ScientificMethodValidator implements ModeValidator<ScientificMethodThought> {
  ...
  validate(
    ...
    if (thought.researchQuestion) {
      if (
        !thought.researchQuestion.question ||
        thought.researchQuestion.question.trim() === ""
      ) {
        issues.push({
          severity: "error",
      ...
      if (thought.researchQuestion.variables) {
        if (
          !thought.researchQuestion.variables.independent ||
          ...
          issues.push({
            ...
            thoughtNumber: thought.thoughtNumber,
            description:
              "Research question should specify independent variables",
        ...
        if (
          !thought.researchQuestion.variables.dependent ||
    ...
    if (
      ...
      if (!hasNull && !hasAlternative) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Consider specifying both null and alternative hypotheses",
    ...
    if (thought.experiment) {
      ...
      if (thought.experiment.sampleSize < 30) {
        issues.push({
          ...
          suggestion:
            "Consider if sample size is adequate for statistical power",
          category: "logical",
        });
      }
      ...
      // Validate variables
      if (
        !thought.experiment.independentVariables ||
      ...
      if (
        !thought.experiment.dependentVariables ||
    ...
    if (thought.data) {
      if (thought.data.dataQuality) {
        if (
          thought.data.dataQuality.completeness < 0 ||
          thought.data.dataQuality.completeness > 1
        ) {
          issues.push({
            severity: "error",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (
          thought.data.dataQuality.reliability < 0 ||
          thought.data.dataQuality.reliability > 1
        ) {
          issues.push({
            severity: "error",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (
          thought.data.dataQuality.validity < 0 ||
          thought.data.dataQuality.validity > 1
        ) {
          issues.push({
            severity: "error",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.analysis) {
      ...
      if (thought.analysis.powerAnalysis) {
        if (
          thought.analysis.powerAnalysis.power < 0 ||
          thought.analysis.powerAnalysis.power > 1
        ) {
          issues.push({
            severity: "error",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.conclusion) {
      ...
      if (
        thought.conclusion.confidence < 0 ||
        thought.conclusion.confidence > 1
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
