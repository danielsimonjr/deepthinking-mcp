  ...
  validate(
    ...
    if (thought.variables && thought.variables.length > 0) {
      ...
      for (const variable of thought.variables) {
        ...
        if (variable.domain) {
          if (
            variable.domain.type === "continuous" ||
            variable.domain.type === "integer"
          ) {
            if (
              "lowerBound" in variable.domain &&
              "upperBound" in variable.domain
            ) {
          ...
          if (
            variable.domain.type === "discrete" &&
            "values" in variable.domain
          ) {
            if (
              !variable.domain.values ||
              variable.domain.values.length === 0
            ) {
              issues.push({
                severity: "error",
                thoughtNumber: thought.thoughtNumber,
          ...
          if (
            variable.domain.type === "categorical" &&
            "categories" in variable.domain
          ) {
            if (
              !variable.domain.categories ||
              variable.domain.categories.length === 0
            ) {
              issues.push({
                severity: "error",
                thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.objectives && thought.objectives.length > 0) {
      ...
      for (const objective of thought.objectives) {
        ...
        if (!objective.formula || objective.formula.trim() === "") {
          issues.push({
            ...
            suggestion:
              "Provide mathematical expression for objective function",
            category: "structural",
          });
        }
      ...
      if (
        ...
        issues.push({
          ...
          suggestion:
            "For multi-objective optimization, weights should sum to 1",
          category: "logical",
        });
      }
    } else {
    ...
    if (thought.solution) {
      ...
      const constraintIds = new Set(
        thought.optimizationConstraints?.map((c) => c.id) || [],
      );
      ...
      for (const varId of variableIds) {
        if (!assignedVars.includes(varId)) {
          issues.push({
            ...
            suggestion:
              "Ensure solution assigns values to all decision variables",
            category: "structural",
          });
        }
      }
      ...
      if (
        thought.solution.computationTime !== undefined &&
        thought.solution.computationTime < 0
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.analysis) {
      ...
      if (thought.analysis.parameters) {
        for (const param of thought.analysis.parameters) {
          if (
            param.allowableIncrease !== undefined &&
            param.allowableIncrease < 0
          ) {
            issues.push({
              severity: "warning",
              thoughtNumber: thought.thoughtNumber,
          ...
          if (
            param.allowableDecrease !== undefined &&
            param.allowableDecrease < 0
          ) {
            issues.push({
              severity: "warning",
              thoughtNumber: thought.thoughtNumber,
