...
export class RecursiveValidator implements ModeValidator<Thought> {
  ...
  validate(thought: Thought, _context: ValidationContext): ValidationIssue[] {
    ...
    if (thought.thoughtNumber <= 2) {
      const hasBaseCase =
        thought.content.toLowerCase().includes("base case") ||
    ...
    if (!hasRecursiveContent) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Recursive reasoning should make recursive structure explicit",
        suggestion:
          "Describe how the problem breaks down into smaller instances",
        category: "structural",
      });
    }
    ...
    if ("dependencies" in thought && Array.isArray(thought.dependencies)) {
      if (thought.thoughtNumber > 2 && thought.dependencies.length === 0) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Recursive steps should depend on previous subproblems or base cases",
    ...
    if (thought.thoughtNumber > 2) {
      const hasCombination =
        thought.content.toLowerCase().includes("combine") ||
      ...
      if (!hasCombination && thought.nextThoughtNeeded) {
        issues.push({
          ...
          suggestion:
            "Describe how to build the final solution from recursive calls",
          category: "logical",
        });
      }
    }
    ...
    if (thought.thoughtNumber > thought.totalThoughts * 0.8) {
      const hasTermination =
        thought.content.toLowerCase().includes("terminat") ||
      ...
      if (!hasTermination && thought.nextThoughtNeeded) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Deep recursion detected - ensure termination conditions are met",
