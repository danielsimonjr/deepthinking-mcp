  ...
  validate(
    ...
    if (thought.causalGraph) {
      ...
      for (const edge of edges) {
        ...
        issues.push(
          ...validateConfidence(
            thought,
            edge.confidence,
            `Edge confidence (${edge.from} -> ${edge.to})`,
          ),
        );
      }
      ...
      if (hasCycle) {
        ...
        const hasFeedbackMechanism = thought.mechanisms?.some(
          (m) => m.type === "feedback",
        );
      ...
      for (const node of nodes) {
        if (!connectedNodes.has(node.id) && nodes.length > 1) {
          issues.push({
            ...
            suggestion:
              "Consider adding causal relationships or removing isolated nodes",
            category: "structural",
          });
        }
      }
    ...
    if (thought.interventions) {
      const nodeIds = new Set(
        thought.causalGraph?.nodes.map((n) => n.id) || [],
      );
