...
export class CritiqueValidator implements ModeValidator<CritiqueThought> {
  ...
  validate(
    ...
    if (thought.methodologyEvaluation) {
      ...
      if (evaluation.design?.rating !== undefined) {
        issues.push(
          ...validateProbability(
            thought,
            evaluation.design.rating,
            "design assessment rating",
          ),
        );
      }
      ...
      if (evaluation.sample?.rating !== undefined) {
        issues.push(
          ...validateProbability(
            thought,
            evaluation.sample.rating,
            "sample assessment rating",
          ),
        );
      }
    ...
    if (thought.verdict) {
      ...
      if (
        thought.verdict.recommendation &&
        !validRecommendations.includes(thought.verdict.recommendation)
      ) {
        issues.push({
          severity: "warning",
          thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.balanceRatio !== undefined) {
      issues.push(
        ...validateProbability(thought, thought.balanceRatio, "balance ratio"),
      );
    }

    // Validate strengths/weaknesses identified are non-negative
    if (
      thought.strengthsIdentified !== undefined &&
      thought.strengthsIdentified < 0
    ) {
      issues.push({
        severity: "error",
        thoughtNumber: thought.thoughtNumber,
    ...
    if (
      thought.weaknessesIdentified !== undefined &&
      thought.weaknessesIdentified < 0
    ) {
      issues.push({
        severity: "error",
        thoughtNumber: thought.thoughtNumber,
