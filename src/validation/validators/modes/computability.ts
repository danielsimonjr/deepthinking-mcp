...
export class ComputabilityValidator implements ModeValidator<ComputabilityThought> {
  ...
  validate(
    ...
    if (thought.currentMachine) {
      issues.push(
        ...this.validateTuringMachine(thought, thought.currentMachine),
      );
    }
    ...
    if (thought.decidabilityProof) {
      issues.push(
        ...this.validateDecidabilityProof(thought, thought.decidabilityProof),
      );
    }
    ...
    if (thought.diagonalization) {
      issues.push(
        ...this.validateDiagonalization(thought, thought.diagonalization),
      );
    }
  ...
  private validateTuringMachine(
    ...
    const overlap = machine.acceptStates.filter((s) =>
      ...
      for (const transition of machine.transitions) {
        ...
        if (transitionKeys.has(key)) {
          issues.push({
            ...
            suggestion:
              "Deterministic machines must have unique transitions per (state, symbol) pair",
            category: "logical",
          });
        }
  ...
  private validateReduction(
    ...
    if (!reduction.correctnessProof.backwardDirection) {
      issues.push({
        ...
        suggestion:
          "Prove that the reduction preserves answers in both directions",
        category: "structural",
      });
    }
  ...
  private validateDecidabilityProof(
    thought: ComputabilityThought,
    proof: DecidabilityProof,
  ): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (proof.conclusion === "undecidable" && proof.method === "reduction") {
      if (!proof.reduction) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Reduction-based undecidability proof requires a reduction",
      ...
      if (!proof.knownUndecidableProblem) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Reduction proof should specify the known undecidable problem",
          suggestion:
            "Identify the undecidable problem being reduced from (e.g., halting problem)",
          category: "structural",
        });
      }
    }
    ...
    if (proof.method === "rice_theorem" && proof.riceApplication) {
      if (!proof.riceApplication.isNontrivial) {
        issues.push({
          ...
          suggestion:
            "Verify the property is non-trivial (some TMs have it, some don't)",
          category: "logical",
        });
      }
      if (!proof.riceApplication.isSemantic) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Rice's theorem only applies to semantic (language) properties",
          suggestion:
            "Verify the property is about the language, not the machine structure",
          category: "logical",
        });
      }
    }
  ...
  private validateDiagonalization(
    thought: ComputabilityThought,
    diag: DiagonalizationArgument,
  ): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (!diag.enumeration.description) {
      issues.push({
        ...
        suggestion:
          "Describe what is being enumerated (e.g., all Turing machines)",
        category: "structural",
      });
    }
    ...
    if (!diag.diagonalConstruction.rule) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Diagonalization argument lacks diagonal construction rule",
  ...
  private validateComputationTrace(
    thought: ComputabilityThought,
  ): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (
      (trace.result === "accept" || trace.result === "reject") &&
      !trace.isTerminating
    ) {
      issues.push({
        severity: "error",
        thoughtNumber: thought.thoughtNumber,
        description:
          "Computation that accepts/rejects should be marked as terminating",
  ...
  private validateComplexityAnalysis(
    thought: ComputabilityThought,
  ): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (analysis.hardnessResults) {
      if (
        analysis.hardnessResults.completeFor &&
        !analysis.hardnessResults.hardFor
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
          ...
          suggestion:
            "A problem that is complete for a class must also be hard for that class",
          category: "logical",
        });
      }
    }
