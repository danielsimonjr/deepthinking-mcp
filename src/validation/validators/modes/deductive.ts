  ...
  validate(
    ...
    if (thought.validityCheck === false) {
      issues.push({
        ...
        suggestion:
          "Review logical structure - invalid deductions should be corrected",
        category: "logical",
      });
    }

    // Warning if valid but unsound
    if (thought.validityCheck === true && thought.soundnessCheck === false) {
      issues.push({
        severity: "warning",
        thoughtNumber: thought.thoughtNumber,
        description: "Deduction is valid but unsound (false premises)",
        suggestion:
          "Valid deduction with false premises does not guarantee true conclusion",
        category: "logical",
      });
    }
