  ...
  validate(
    ...
    if (thought.logicalInferences && thought.logicalInferences.length > 0) {
      const propositionIds = new Set(
        thought.propositions?.map((p) => p.id) || [],
      );
            ...
            for (const refStep of step.referencesSteps) {
              if (!stepNumbers.has(refStep) && refStep < step.stepNumber) {
                ...
                issues.push({
                  ...
                  suggestion:
                    "Ensure referenced steps exist and come before current step",
                  category: "logical",
                });
              }
            }
    ...
    if (thought.truthTable) {
      if (
        !thought.truthTable.propositions ||
        thought.truthTable.propositions.length === 0
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
      ...
      const expectedRows = Math.pow(
        2,
        thought.truthTable.propositions?.length || 0,
      );
      if (
        thought.truthTable.rows &&
        thought.truthTable.rows.length !== expectedRows
      ) {
        issues.push({
          severity: "warning",
          thoughtNumber: thought.thoughtNumber,
      ...
      if (thought.truthTable.rows) {
        const allTrue = thought.truthTable.rows.every(
          (row) => row.result === true,
        );
        const allFalse = thought.truthTable.rows.every(
          (row) => row.result === false,
        );
        ...
        if (allTrue && !thought.truthTable.isTautology) {
          issues.push({
            ...
            thoughtNumber: thought.thoughtNumber,
            description:
              "All rows evaluate to true but not marked as tautology",
        ...
        if (allFalse && !thought.truthTable.isContradiction) {
          issues.push({
            ...
            thoughtNumber: thought.thoughtNumber,
            description:
              "All rows evaluate to false but not marked as contradiction",
        ...
        if (!allTrue && !allFalse && !thought.truthTable.isContingent) {
          issues.push({
            ...
            thoughtNumber: thought.thoughtNumber,
            description:
              "Formula has mixed truth values but not marked as contingent",
    ...
    if (thought.satisfiability) {
      if (
        !thought.satisfiability.formula ||
        thought.satisfiability.formula.trim() === ""
      ) {
        issues.push({
          severity: "error",
          thoughtNumber: thought.thoughtNumber,
      ...
      if (thought.satisfiability.satisfiable && !thought.satisfiability.model) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Satisfiable formula should provide satisfying assignment",
          suggestion:
            "Include model (truth assignment) that satisfies the formula",
          category: "structural",
        });
      }
