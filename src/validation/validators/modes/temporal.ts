  ...
  validate(
    ...
    if (thought.timeline) {
      if (!thought.timeline.timeUnit) {
        issues.push({
          ...
          suggestion:
            "Add timeUnit property (milliseconds, seconds, minutes, hours, days, months, years)",
    ...
    if (thought.constraints) {
      ...
      for (const constraint of thought.constraints) {
        if (
          !eventIds.has(constraint.subject) &&
          !intervalIds.has(constraint.subject)
        ) {
          issues.push({
            severity: IssueSeverity.ERROR,
            thoughtNumber: thought.thoughtNumber,
        ...
        if (
          !eventIds.has(constraint.object) &&
          !intervalIds.has(constraint.object)
        ) {
          issues.push({
            severity: IssueSeverity.ERROR,
            thoughtNumber: thought.thoughtNumber,
        ...
        issues.push(
          ...validateConfidence(
            ...
            `Constraint ${constraint.id} confidence`,
          ),
        );
      }
    }
