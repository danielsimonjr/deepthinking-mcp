  ...
  validate(
        ...
        if (source.quality) {
          if (source.quality.methodologicalRigor !== undefined) {
            issues.push(
              ...validateProbability(
                thought,
                source.quality.methodologicalRigor,
                `source "${source.id}" methodological rigor`,
              ),
            );
          }
          ...
          if (source.quality.authorCredibility !== undefined) {
            issues.push(
              ...validateProbability(
                thought,
                source.quality.authorCredibility,
                `source "${source.id}" author credibility`,
              ),
            );
          }
          if (source.quality.overallQuality !== undefined) {
            issues.push(
              ...validateProbability(
                thought,
                source.quality.overallQuality,
                `source "${source.id}" overall quality`,
              ),
            );
          }
        }
        ...
        if (concept.importance !== undefined) {
          issues.push(
            ...validateProbability(
              thought,
              concept.importance,
              `concept "${concept.term}" importance`,
            ),
          );
        }
    ...
    if (thought.themes && thought.themes.length > 0) {
      ...
      for (const theme of thought.themes) {
        ...
        if (
          theme.consensus &&
          !validConsensusLevels.includes(theme.consensus)
        ) {
          issues.push({
            severity: "info",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.findings && thought.findings.length > 0) {
      ...
      const validReplicationStatuses = [
        "replicated",
        "partial",
        "not_replicated",
        "unknown",
      ...
      for (const finding of thought.findings) {
        if (
          ...
          !validEvidenceStrengths.includes(finding.evidenceStrength)
        ) {
          issues.push({
            severity: "warning",
            thoughtNumber: thought.thoughtNumber,
        ...
        if (
          ...
          !validReplicationStatuses.includes(finding.replicationStatus)
        ) {
          issues.push({
            severity: "info",
            thoughtNumber: thought.thoughtNumber,
    ...
    if (thought.framework) {
      ...
      if (
        thought.framework.relationships &&
        thought.framework.relationships.length > 0
      ) {
        const validRelTypes = [
          "causes",
          "correlates",
