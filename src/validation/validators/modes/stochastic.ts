  ...
  validate(thought: Thought, _context: ValidationContext): ValidationIssue[] {
      ...
      if (typeof dist === "object") {
        if (!dist.type) {
          issues.push({
            ...
            suggestion:
              "Specify distribution type (e.g., normal, uniform, exponential)",
            category: "structural",
          });
        }
        ...
        if (dist.parameters && typeof dist.parameters === "object") {
          ...
          if (
            dist.type === "normal" &&
            (!dist.parameters.mean || !dist.parameters.stddev)
          ) {
            issues.push({
              severity: "error",
              thoughtNumber: thought.thoughtNumber,
              description:
                "Normal distribution requires mean and stddev parameters",
    ...
    if ("uncertainty" in thought && typeof thought.uncertainty === "number") {
      ...
      if (thought.uncertainty === 0) {
        issues.push({
          ...
          thoughtNumber: thought.thoughtNumber,
          description:
            "Stochastic reasoning usually involves non-zero uncertainty",
          suggestion:
            "Consider quantifying the uncertainty in your stochastic process",
          category: "logical",
        });
      }
      ...
      issues.push({
        ...
        suggestion:
          "Stochastic reasoning benefits from explicit uncertainty measures",
        category: "structural",
      });
    }
    ...
    if (!hasStochasticContent) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Stochastic reasoning should discuss random processes or distributions",
        suggestion:
          "Make explicit references to probability distributions or random variables",
        category: "structural",
      });
    }
