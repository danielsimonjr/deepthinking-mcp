  ...
  validate(thought: Thought, _context: ValidationContext): ValidationIssue[] {
    ...
    if (!hasModalOperator) {
      issues.push({
        ...
        description:
          "Modal logic should use modal operators (necessary, possible, impossible)",
        suggestion:
          "Express claims using modalities: □ (necessity), ◇ (possibility)",
        category: "structural",
      });
    }
    ...
    if (!hasWorldReference && thought.thoughtNumber > 1) {
      issues.push({
        ...
        thoughtNumber: thought.thoughtNumber,
        description:
          "Modal reasoning often considers different possible worlds or scenarios",
        suggestion:
          "Consider making explicit references to alternative scenarios or states",
        category: "structural",
      });
    }
