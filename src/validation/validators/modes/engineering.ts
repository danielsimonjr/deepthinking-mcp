  ...
  validate(
    ...
    if (thought.analysisType === "comprehensive") {
      const hasAny =
        ...
        thought.tradeStudy ||
        thought.fmea ||
        thought.designDecisions;
      if (!hasAny) {
        issues.push({
          severity: "warning",
          thoughtNumber: thought.thoughtNumber,
          description:
            "Comprehensive analysis should include at least one analysis section",
          suggestion:
            "Add requirements, trade study, FMEA, or design decisions",
          category: "structural",
        });
      }
    }
...
function validateRequirements(thought: EngineeringThought): ValidationIssue[] {
      ...
      for (const parentId of req.tracesTo) {
        if (!ids.includes(parentId)) {
          issues.push({
            ...
            suggestion:
              "Verify traceability links point to valid requirement IDs",
            category: "structural",
          });
        }
      }
  ...
  const reqsWithVerification = reqs.requirements.filter(
...
function validateTradeStudy(thought: EngineeringThought): ValidationIssue[] {
  ...
  for (const altId of altIds) {
    for (const critId of critIds) {
      const score = study.scores.find(
        (s) => s.alternativeId === altId && s.criteriaId === critId,
      );
      if (!score) {
        issues.push({
          severity: "warning",
          ...
          suggestion:
            "Provide scores for all alternative-criterion combinations",
          category: "structural",
        });
      }
    }
  ...
  if (study.alternatives.length < 2) {
    issues.push({
      ...
      suggestion:
        "Consider evaluating multiple alternatives for a meaningful trade study",
      category: "structural",
    });
  }
...
function validateFMEA(thought: EngineeringThought): ValidationIssue[] {
  ...
  const criticalCount = fmea.failureModes.filter(
    (fm) => fm.rpn >= fmea.rpnThreshold,
  ).length;
