...
const SYSTEMS_ARCHETYPES: SystemArchetype[] = [
  {
    ...
    description:
      "A quick fix creates unintended consequences that make the problem worse",
    pattern:
      "Solution → Short-term relief → Unintended consequences → Original problem worsened",
    warningSigns: [
  ...
  },
  {
    ...
    pattern:
      "Symptom → Quick fix AND Fundamental solution, Quick fix erodes capacity for fundamental solution",
    warningSigns: [
  ...
  },
  {
    ...
    description:
      "Growth approaches a limit which slows or stops further growth",
    pattern:
      "Growth process → Success → Limiting condition activated → Growth slows/stops",
    warningSigns: [
  ...
  },
  {
    ...
    pattern:
      "Initial advantage → More resources allocated → Greater success → Competitors starved",
    warningSigns: [
  ...
  },
  {
    ...
    pattern:
      "Shared resource → Individual exploitation → Resource depletion → Harm to all",
    warningSigns: [
  ...
  },
  {
    ...
    pattern:
      "Party A acts → Party B perceives threat → Party B responds → Party A perceives threat → Cycle",
    warningSigns: [
  ...
  },
  {
    ...
    pattern:
      "Demand grows → Capacity stretched → Performance drops → Investment deferred → Limits growth",
    warningSigns: [
  ...
  },
  {
    name: "Eroding Goals",
    description:
      "Under pressure, goals are lowered rather than addressing the gap",
    pattern:
      "Gap between goals and reality → Pressure → Goals lowered → Gap closed artificially",
    warningSigns: [
...
export class SystemsThinkingHandler implements ModeHandler {
  ...
  readonly description =
    "Systems analysis with archetype detection, feedback loops, and leverage point identification";
  ...
  supportsThoughtType(thoughtType: string): boolean {
    return this.supportedThoughtTypes.includes(
      thoughtType as SystemsThinkingThoughtType,
    );
  }
  ...
  ): SystemsThinkingThoughtType {
    if (
      inputType &&
      this.supportedThoughtTypes.includes(
        inputType as SystemsThinkingThoughtType,
      )
    ) {
  ...
  private validateFeedbackLoop(
    ...
    // Validate strength
    if (
      loop.strength !== undefined &&
      (loop.strength < 0 || loop.strength > 1)
    ) {
      warnings.push(
        createValidationWarning(
  ...
  private validateLeveragePoint(
    ...
    if (
      lp.effectiveness !== undefined &&
      (lp.effectiveness < 0 || lp.effectiveness > 1)
    ) {
      warnings.push(
        createValidationWarning(
    ...
    if (
      ...
      (lp.difficulty < 0 || lp.difficulty > 1)
    ) {
      warnings.push(
        createValidationWarning(
