 ...
 */
const METHODOLOGY_GUIDANCE: Record<
  AnalysisMethodology,
  { description: string; keySteps: string[] }
> = {
  thematic_analysis: {
...
export class AnalysisHandler implements ModeHandler {
  ...
  readonly description =
    "Rigorous qualitative analysis with codebook validation and saturation assessment";
  ...
  createThought(input: ThinkingToolInput, sessionId: string): AnalysisThought {
    ...
    // Assess rigor
    const rigorAssessment =
      inputAny.rigorAssessment || this.assessRigor(inputAny);
    ...
    return {
      ...
      totalSegments:
        inputAny.dataSegments?.length || inputAny.totalSegments || 0,
      codebook,
  ...
  validate(input: ThinkingToolInput): ValidationResult {
    ...
    // Check methodology
    if (
      inputAny.methodology &&
      !Object.keys(METHODOLOGY_GUIDANCE).includes(inputAny.methodology)
    ) {
      warnings.push(
        createValidationWarning(
          "methodology",
    ...
    if (inputAny.themes && inputAny.themes.length > 0) {
      const saturatedThemes = inputAny.themes.filter(
        (t: QualitativeTheme) => t.prevalence > 0.7,
      );
      const sparseThemes = inputAny.themes.filter(
        (t: QualitativeTheme) => t.prevalence < 0.2,
      );
  ...
  getEnhancements(thought: AnalysisThought): ModeEnhancements {
    ...
    enhancements.metrics = {
      ...
      avgThemePrevalence:
        themes.length > 0
          ? themes.reduce((sum, t) => sum + t.prevalence, 0) / themes.length
          : 0,
    };
    ...
    if (thoughtType === "saturation_assessment") {
      ...
      if (
        rigor?.saturation?.newCodesLastN !== undefined &&
        rigor.saturation.newCodesLastN > 3
      ) {
        enhancements.suggestions!.push(
  ...
  supportsThoughtType(thoughtType: string): boolean {
    return this.supportedThoughtTypes.includes(
      thoughtType as AnalysisThoughtType,
    );
  }
  ...
  private resolveThoughtType(
    ...
    if (
      inputType &&
      this.supportedThoughtTypes.includes(inputType as AnalysisThoughtType)
    ) {
  ...
  private calculateCodingProgress(
    ...
    const percentComplete =
      totalSegments > 0 ? (segmentsCoded / totalSegments) * 100 : 0;
  ...
  private assessRigor(input: any): QualitativeRigor {
    const hasMultipleCoders =
      input.codebook?.intercoderReliability !== undefined;
    const hasThickDescription = input.themes?.some(
      (t: QualitativeTheme) => t.keyQuotes?.length > 2,
    );
    const hasMemos = input.memos && input.memos.length > 0;
    const hasReflexiveMemos = input.memos?.some(
      (m: any) => m.type === "reflective_memo",
    );
