...
export interface CorrectnessProof {
  ...
  // Proof method
  method:
    "loop_invariant" | "induction" | "contradiction" | "direct" | "structural";
...
export interface GraphAlgorithmContext {
  ...
  // Representation used
  representation:
    "adjacency_list" | "adjacency_matrix" | "edge_list" | "implicit";
...
export function suggestDesignPattern(characteristics: {
  ...
  if (
    characteristics.hasOptimalSubstructure &&
    characteristics.overlappingSubproblems
  ) {
    return "dynamic_programming";
  }
  if (
    characteristics.hasOptimalSubstructure &&
    characteristics.greedyChoiceProperty
  ) {
    return "greedy";
  }
 ...
 */
export const COMMON_RECURRENCES: Record<
  string,
  { recurrence: string; solution: string; example: string }
> = {
  binary_search: {
