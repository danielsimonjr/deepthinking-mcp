...
export const ThinkingToolSchema = z.object({
  ...
  dependencies: z
    .array(z.string().max(MAX_LENGTHS.DESCRIPTION))
  ...
  observations: z
    .union([
      z
        .array(z.string().max(MAX_LENGTHS.DESCRIPTION))
        .max(MAX_LENGTHS.ARRAY_ITEMS), // For inductive reasoning - simple strings
      z
        .array(
          z.object({
  ...
  scientificConclusion: z
    .object({
      id: z.string().max(MAX_LENGTHS.DESCRIPTION),
      statement: z.string().max(MAX_LENGTHS.DESCRIPTION),
      supportedHypotheses: z
        .array(z.string().max(MAX_LENGTHS.DESCRIPTION))
        .max(MAX_LENGTHS.ARRAY_ITEMS),
      rejectedHypotheses: z
        .array(z.string().max(MAX_LENGTHS.DESCRIPTION))
        .max(MAX_LENGTHS.ARRAY_ITEMS),
      confidence: z.number().min(0).max(1),
      limitations: z
  ...
  solution: z
    .object({
      ...
      variableValues: boundedRecord(
        z.string().max(MAX_LENGTHS.DESCRIPTION),
        z.union([z.number(), z.string().max(MAX_LENGTHS.DESCRIPTION)]),
      ),
      objectiveValues: boundedRecord(
        z.string().max(MAX_LENGTHS.DESCRIPTION),
        z.number(),
      ),
  ...
  sensitivityAnalysis: z
    .object({
      ...
      criticalConstraints: z
        .array(z.string().max(MAX_LENGTHS.DESCRIPTION))
        .max(MAX_LENGTHS.ARRAY_ITEMS),
      shadowPrices: boundedRecord(
        z.string().max(MAX_LENGTHS.DESCRIPTION),
        z.number(),
      ).optional(),
      recommendations: z
